...
export function ProductImageManager(props: { productId: string }) {
	const [images, setImages] = createSignal<ProductImageResponse[]>([]);
	...
	const fetchImages = async () => {
		const res = await apiFetch<{
			...
			status: number;
		}>(`/api/platform/products/${props.productId}/images`);
	...
	const handleFileSelect = async (e: Event) => {
		...
		try {
			...
			const res = await apiFetch<{
				data: ProductImageResponse & { error?: string };
				status: number;
			}>(`/api/platform/products/${props.productId}/images`, {
				method: "POST",
				body: formData,
	...
	const handleDelete = async (imageId: string) => {
		const res = await apiFetch<{ status: number }>(
			`/api/platform/products/${props.productId}/images/${imageId}`,
			{ method: "DELETE" },
		);
		if (res.status === 204) {
